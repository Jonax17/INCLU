...
export default function LectorTexto() {
  ...
  const captureAndRecognize = useCallback(async () => {
    ...
    try {
      ...
      const worker = await createWorker('spa+eng', 1, {
        logger: (m) => {
          if (m.status === 'recognizing text') setProgress(`Reconociendo: ${Math.round(m.progress * 100)}%`);
        }
      });

      setProgress('Reconociendo texto...');
      ...
      if (cleanText) {
        ...
        setRecognized('');
        setError('No se detectó texto en la imagen. Apunta a un texto visible.');
        setProgress('');
      }
    } catch {
  ...
  const speakText = () => {
    if (recognized && 'speechSynthesis' in window) {
  ...
  return (
    <AppLayout>
      ...
      <p>Apunta la cámara a un texto y reconócelo con OCR real.</p>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
          {!cameraActive ? (
            ...
            <>
              <button className="btn btn-p" onClick={captureAndRecognize} disabled={scanning}>
                {scanning ? 'Procesando...' : 'Leer texto'}
              </button>
      ...
      {recognized && (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
            <button className="btn btn-s" onClick={speakText}>Leer en voz alta</button>
            <button className="btn btn-o" onClick={() => window.speechSynthesis?.cancel()}>Detener voz</button>
            <button className="btn btn-o" onClick={() => setRecognized('')}>Borrar</button>
          </div>
        </div>
      )}
    </AppLayout>
